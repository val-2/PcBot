...
def send_message(update: telegram.Update, text, log_level=logging.INFO, parse_mode=None, disable_web_page_preview=None, disable_notification=False, reply_to_message_id=None, reply_markup=None, timeout=None, **kwargs):
    while True:
        try:
            ...
            log_msg = f'Message to {update.message.chat_id}: "{text}"'
            ...
            if str(e) == "urllib3 HTTPError [SSL: DECRYPTION_FAILED_OR_BAD_RECORD_MAC] decryption failed or bad record mac (_ssl.c:2508)":
                update.message.bot.send_message(f'Network error while sending message "{text}" to {update.message.chat.first_name}. Retrying...')
